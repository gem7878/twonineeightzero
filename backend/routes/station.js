...
router.get("/maps/:sLine/:sName", async function(req,res,next) {
    ...
    elevator1.forEach(row => {
        ...
        let floors = row.상세위치.match(/\(.*\)/)[0].match(/\w\d/g).sort();
    ...
    return res.json({'전역':{'호선':전역.호선, '전철역명':전역.전철역명, '에스컬레이터': es_list1, '엘레베이터': el_list1, '장애인화장실': ta_list1, '화장실': tb_list1},'후역':{'호선':후역.호선, '전철역명':후역.전철역명, '에스컬레이터': es_list2, '엘레베이터': el_list2, '장애인화장실': ta_list2, '화장실': tb_list2}});
})
...
function getStationId(stationLine, stationName) {
    const results = [];
    const stream = fs.createReadStream('./db/서울교통공사 노선별 지하철역 정보.csv')
        .pipe(csv({ columns: true}))
        .pipe(es.map(function(line, cb){
            if(line['전철역명'].includes(stationName) && line['호선'].includes(stationLine)){ // '5' or '경의선' 등.. 
                results.push(line);
            }            
            cb(null, line)
        }));
...
function allfloor(array) {
    const all = [];
    array.forEach(item => {
        for(var idx = item[1]; idx > 0; idx--) all.push((item[0] === "B" ? '지하': '지상')+idx+'층');
    })
    return all;
}
